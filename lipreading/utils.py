...
# -- logging utils
def get_logger(args,save_path, num_classes):
    log_path = '{}/{}_{}_{}classes_log.txt'.format(save_path,args.training_mode,args.lr,num_classes)
...
def get_save_folder( args):
    ...
    save_path += '/' + 'cont_train_resnet18_mstcn_50cls_v1_' + datetime.datetime.now().isoformat().split('.')[0]
    save_path = save_path.replace("/", "\\").replace("-", "_").replace(":", "_")
